...
INTROS = {
    CONVERSATION_ID: (
        "conversation",
        "A page taller than the screen. The device holds all of it and shows one\n"
        "screen at a time, so a page turn is answered on the device rather than by\n"
        "asking the host for another picture.",
    ),
    DIFF_ID: (
        "changed files",
        "The second view, sent because the device forwarded a swipe. This one\n"
        "arrives over the wire, which is the round trip the local answers avoid.",
    ),
}
