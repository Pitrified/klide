...
- Use the PW5 already in hand, jailbroken with LanguageBreak (D3, D4). No Amazon account and no wifi needed for the exploit. It ships on 5.14.1.1 and is updated offline to 5.16.2.1.1 first, the version LanguageBreak's instructions ask for (Q2, revised).
...
- 2026-09-04 : re-read the LanguageBreak README on the firmware question and reversed Q2. It says to upgrade to 5.16.2.1.1 before starting, in the Warnings block and again as the fix for general failures, and the current kindlemodding page gives only the ceiling with no upgrade advice at all. Phase 1 now updates offline first and jailbreaks second. Also resolved the archive unknown: the PW5 5.16.2.1.1 image is on Amazon's S3 (HTTP 200, 299464378 bytes), with the community archive kept as a second copy to checksum against since Amazon publishes none. The update is one-way until rooted, because downgrading requires a jailbreak
